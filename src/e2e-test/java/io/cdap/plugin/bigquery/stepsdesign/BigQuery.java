...
public class BigQuery implements CdfHelper {
  ...
  @Then("Validate partition date of table {string} is equal to " +
    "partitioned start date in output partitioned table {string}")
  public void validatePartitionDateOfTableIsEqualToPartitionedStartDateInOutputPartitionedTable
    (String inputTable, String outputTable) throws IOException, InterruptedException {
    ...
    String outputDate = GcpClient.executeSelectQuery("SELECT distinct  _PARTITIONDATE as pt FROM `" +
                                                       ...
                                                       + "` WHERE _PARTITION_LOAD_TIME IS Not NULL ");
    BeforeActions.scenario.write("partitioned start date in output record:"
                                   + outputDate);
